...
# Day 16 (29-01)
I added Europe as an option in the dropdown menu. When selected, the entire sunburst will be displayed. Because of this, that option is now the default when the site is loaded. I also changed the barchart data from selecting the 'Total' from the dataset to adding up all the other numbers. This is because I discovered an error in the data, where the total would not be equal to the sum of it's given parts. I made the sunburst tooltip to display '100%' when hovering over Europe. I made the map bigger and displayed the gradient legend of the map upside down, so it goes from highest to lowest value. I am working on a highlight function that will highlight a country when the mouse is hovering over the corresponding bar.  
  
# Day 17 (30-01)
I made the map so that when a country without data is clicked, the entire sunburst (so for the whole of Europe) is shown. Previously, the sunburst still displayed the country that was selected before and did not change at all. The dropdown menu would previously display nothing but emptiness. Now it displays the string option "Europe".  
