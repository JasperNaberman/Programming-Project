...
## Visual Design  
For the visual design of this product I used a lot of Bootstrap, which I learned from w3schools.com. However, I did not use a pre-made template. I chose for a navigation bar at each page because I think it gives a clear overview of what pages the site has to offer. At this visualization page I chose 2 color scales to represent the data. The sunburst and bar chart both use colors from a purple color scheme, because they both represent absolute values of immigrants. The bar chart is however not color coded, so the color is displays is purely for aesthetic purposes. For the map I chose a different color scheme since it displays different data, namely the ratio of immigrants per 1000 capita. I am not colorblind myself but because this design should be colorblind-friendly I installed a program named "Sim Daltonism", which simulated onscreen what a colorblind person (8 different kinds of colorblindness) would see when looking at the website. Because of this program I was able to check if the site was colorblind-friendly.
